...
class ItineraryCubit extends Cubit<ItineraryState> {
  ...
  Future<void> loadItinerary(String groupId, {bool silent = false}) async {
    ...
    if (kDebugMode) dev.log('[CUBIT] loadItinerary: groupId=$groupId silent=$silent', name: 'itinerary');
    if (!silent) {
    ...
    await groupResult.fold(
      (failure) async {
        ...
        if (kDebugMode) dev.log('[CUBIT] loadItinerary: FALHA getGroupDetails: $failure', name: 'itinerary');
        emit(ItineraryState.error(_mapFailure(failure)));
      },
      (group) async {
        debugPrint('[CUBIT] loadItinerary: grupo carregado: ${group.id} nome=${group.name}');
        if (kDebugMode) dev.log('[CUBIT] loadItinerary: grupo carregado: ${group.id} nome=${group.name}', name: 'itinerary');
        ...
        await itemsResult.fold(
          (failure) async {
            ...
            if (kDebugMode) dev.log('[CUBIT] loadItinerary: FALHA getItinerary: $failure', name: 'itinerary');
            emit(ItineraryState.error(_mapFailure(failure)));
          },
          (items) async {
            ...
            if (kDebugMode) dev.log('[CUBIT] loadItinerary: emitindo LOADED. group=${group.id}', name: 'itinerary');
  ...
  void _subscribeToGrupoChanges(String groupId) {
    ...
    _grupoSubscription = supabase
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            if (kDebugMode) dev.log('[CUBIT] _subscribeToGrupoChanges: EVENTO grupo=$groupId new=${payload.newRecord}', name: 'itinerary');
            loadUserItinerary();
          },
        )
        .subscribe((status, [error]) {
          if (kDebugMode) dev.log('[CUBIT] _subscribeToGrupoChanges: subscription status=$status error=$error', name: 'itinerary');
        });
  }
  ...
  void _subscribeToEventsChanges(String groupId) {
    ...
    _eventsSubscription = supabase
        ...
        .onPostgresChanges(
          ...
          callback: (_) {
            if (kDebugMode) dev.log('Realtime: evento changed in group $groupId. Refreshing...');
  ...
  Future<void> loadUserItinerary() async {
    ...
    if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: chamado. state=${state.runtimeType}', name: 'itinerary');
    ...
    if (!isSilent) {
      emit(const ItineraryState.loading());
      if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: emitido loading', name: 'itinerary');
    } else {
      if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: refresh silencioso (já tem dados)', name: 'itinerary');
    }
    ...
    if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: chamando OnboardingService.refresh()...', name: 'itinerary');
    await OnboardingService.instance.refresh();
    if (isClosed) return;
    if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: needsOnboarding=${OnboardingService.instance.needsOnboarding}', name: 'itinerary');

    if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: chamando getUserGroupId()...', name: 'itinerary');
    ...
    await userGroupResult.fold(
      (failure) async {
        ...
        if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: FALHA no getUserGroupId: $failure', name: 'itinerary');
        emit(ItineraryState.error(_mapFailure(failure)));
      },
      (groupId) async {
        if (isClosed) return;
        debugPrint('[CUBIT] loadUserItinerary: groupId=$groupId');
        if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: groupId=$groupId', name: 'itinerary');
        if (groupId == null) {
          debugPrint('[CUBIT] loadUserItinerary: groupId=null → emitindo error (sem missão ativa)');
          if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: groupId=null → emitindo error (sem missão ativa)', name: 'itinerary');
          activeGroups = [];
          emit(
          ...
          if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: chamando loadItinerary($groupId, silent=$isSilent)...', name: 'itinerary');
          await loadItinerary(groupId, silent: isSilent);
        }
      },
    );
    debugPrint('[CUBIT] loadUserItinerary: finalizado. state final=${state.runtimeType}');
    if (kDebugMode) dev.log('[CUBIT] loadUserItinerary: finalizado. state final=${state.runtimeType}', name: 'itinerary');
  }
  ...
  void listenToGroupChanges() {
    ...
    if (kDebugMode) dev.log('[CUBIT] listenToGroupChanges: userId=$userId', name: 'itinerary');
    ...
    _groupSubscription = supabase
        ...
        .onPostgresChanges(
          ...
          callback: (payload) {
            if (kDebugMode) dev.log('[CUBIT] listenToGroupChanges: EVENTO recebido! eventType=${payload.eventType} new=${payload.newRecord} old=${payload.oldRecord}', name: 'itinerary');
            loadUserItinerary();
          },
        )
        .subscribe((status, [error]) {
          if (kDebugMode) dev.log('[CUBIT] listenToGroupChanges: subscription status=$status error=$error', name: 'itinerary');
        });
  }
